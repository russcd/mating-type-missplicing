...
and gene expression (TPM) per gene, for four species. Runs separate analyses
and scatter plots for autosomal (In_region==0) vs mating_type (In_region==1) genes.
"""
import argparse
import os
...
def process_species_region(tsv_path, in_region):
    ...
    and compute per-gene median TPM and median retention.
    Returns a DataFrame with columns: gene_id, median_tpm, median_retention.
    """
    ...
    gene = gene.dropna(subset=["median_retention", "median_tpm"])
    return gene
...
def run_region(region_name, in_region, args, results_by_region, data_by_region):
    ...
    for short_name, display_name in SPECIES:
        ...
        n_genes = len(gene_df)
        if n_genes < 2:
            print(f"Warning: {display_name} {region_name} has < 2 genes with valid data, skipping")
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Correlate median intron retention per gene with median TPM per gene (autosomal vs mating_type)."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="analysis/intron_retention_vs_tpm.pdf",
        help="Base output path for figures; _autosomal.pdf and _mating_type.pdf will be used (default: analysis/intron_retention_vs_tpm.pdf)",
    )
    ...
    print("Median intron retention rate vs median TPM per gene (Spearman)")
    print()

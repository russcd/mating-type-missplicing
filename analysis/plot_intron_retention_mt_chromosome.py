...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot intron-weighted avg_retention_frequency only for chromosomes containing the mating-type region, for each species."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-r", "--regions", required=True,
        help="File specifying start/stop regions per species (tab/space-delimited)."
    )
    parser.add_argument(
        "-o", "--output", default="genome_retention_mt_chromosome_plot.pdf",
        help="Output plot file (default: genome_retention_mt_chromosome_plot.pdf)."
    )
...
def load_file(file):
    ...
    df["avg_retention_frequency"] = pd.to_numeric(
        df["rMATS_retention_ratio"], errors="coerce"
    ).fillna(0)
...
def main():
    ...
    for file in tqdm(args.files, desc="Processing species files"):
        ...
        if species not in regions_dict:
            print(
                f"Skipping {file}: species '{species}' not found in regions file {args.regions}."
            )
            continue

        df = load_file(file)
        if df.empty:
            print(f"Skipping {file}: no expressed introns.")
            continue

        # Restrict to chromosome(s) that contain the mating-type region
        mt_chroms = set(regions_dict[species].keys())
        df = df[df["chromosome"].astype(str).isin(mt_chroms)]
        if df.empty:
            print(
                f"Skipping {file}: no introns on chromosomes with mating-type regions."
            )
            continue

        windows_df = make_windows(df, args.min_introns)
        if windows_df.empty:
            print(f"Skipping {file}: no valid data after windowing.")
            continue
    ...
    species_with_lengths = [
        (get_genome_length(windows_df), windows_df, raw_df)
    ...
    all_raw_data = [
        (windows_df["species"].iloc[0], raw_df)
        for _, windows_df, raw_df in species_with_lengths
    ]

    max_genome_span = 0
    for windows_df in all_species_data:
        chrom_sizes = (
            windows_df.groupby("chromosome")["end"].max().sort_values(ascending=False)
        )
        gap = 1e6 * 0.2
    ...
    for i, (windows_df, (species, raw_df)) in enumerate(
        ...
        in_region_data, outside_region_data = get_per_intron_data(
            raw_df, regions_dict, species
        )
        ...
        for chrom in tqdm(chrom_order, desc=f"Plotting {species}", leave=False):
            chrom_df = windows_df[windows_df["chromosome"] == chrom].sort_values(
                "midpoint"
            )
            if chrom_df.empty:
                continue
